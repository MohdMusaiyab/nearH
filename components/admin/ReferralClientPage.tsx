...
export default function ReferralClientPage({ initialData, hospitalId }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* ── Filter & Action Bar ── */}
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white p-3 rounded-2xl border border-[var(--color-border)] shadow-sm">
        <div className="flex p-1 bg-[var(--color-badge-bg)]/50 rounded-xl w-full md:w-auto">
          {[
        ...
        <Link
          ...
          className="w-full md:w-auto flex items-center justify-center gap-2 bg-[var(--color-accent)] text-white px-6 py-3 rounded-xl text-sm font-black uppercase tracking-widest hover:bg-[var(--color-accent-hover)] transition-all shadow-lg shadow-[var(--color-accent)]/20 active:scale-[0.98]"
        >
      ...
      <div className="space-y-4">
        {filteredData.length === 0 ? (
          <div className="py-24 text-center bg-white rounded-3xl border border-[var(--color-border)] border-dashed">
            <div className="w-16 h-16 bg-[var(--color-badge-bg)] rounded-full flex items-center justify-center mx-auto mb-4">
              {activeTab === "inbound" ? (
                <Inbox className="text-[var(--color-muted)]" />
              ) : (
                <Send className="text-[var(--color-muted)]" />
              )}
            </div>
            <p className="text-[var(--color-heading)] font-black text-sm uppercase tracking-tight">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-4">
          <button
            ...
            className="p-2.5 rounded-xl border border-[var(--color-border)] bg-white text-[var(--color-muted)] hover:text-[var(--color-accent)] disabled:opacity-30 transition-all shadow-sm"
          >
            <ChevronLeft size={18} />
          </button>

          <div className="px-6 py-2 rounded-xl bg-white border border-[var(--color-border)] text-xs font-black text-[var(--color-heading)] shadow-sm">
            {currentPage}{" "}
