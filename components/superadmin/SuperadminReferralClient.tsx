...
const STATUS_STYLES: Record<Status, { pill: string; dot: string }> = {
  ...
  Rejected: {
    pill: "bg-red-50 text-[var(--color-error)] border border-red-200",
    dot: "bg-[var(--color-error)]",
  },
  Completed: {
    pill: "bg-[var(--color-badge-bg)] text-[var(--color-badge-text)] border border-[var(--color-border)]",
  ...
  return (
    <div className="space-y-4">
      ...
      {initialReferrals.length === 0 && (
        <div className="bg-white rounded-2xl border border-[var(--color-border)] flex flex-col items-center justify-center py-20 text-center px-6">
          <div className="w-14 h-14 rounded-2xl bg-[var(--color-badge-bg)] border border-[var(--color-border)] flex items-center justify-center mb-4">
            <AlertCircle size={24} className="text-[var(--color-muted)]" />
          </div>
      ...
      {initialReferrals.length > 0 && (
        <>
          {/* ── Desktop table ── */}
          <div className="hidden lg:block bg-white rounded-2xl border border-[var(--color-border)] overflow-hidden">
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="min-w-0">
                              <p className="text-xs font-bold text-[var(--color-heading)] truncate max-w-[140px]">
                                {ref.from_hospital?.name}
                              </p>
                              <p className="text-[10px] text-[var(--color-muted)] flex items-center gap-1 font-medium mt-0.5">
                                <MapPin size={10} />
                            ...
                            <ArrowRight
                              size={14}
                              className="text-[var(--color-border)] flex-shrink-0"
                            />
                            <div className="min-w-0">
                              <p className="text-xs font-bold text-[var(--color-heading)] truncate max-w-[140px]">
                        ...
                        <td className="px-5 py-4 text-right">
                          <Link
                            ...
                            className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-white border border-[var(--color-border)] rounded-xl text-xs font-bold text-[var(--color-muted)] hover:text-[var(--color-accent)] hover:border-[var(--color-accent)]/40 hover:bg-[var(--color-badge-bg)] transition-all opacity-0 group-hover:opacity-100"
                          >
          ...
          <div className="lg:hidden space-y-3">
            {initialReferrals.map((ref) => {
              ...
              return (
                ...
                >
                  {/* Card header */}
                  <div className="px-5 py-4 flex items-start justify-between gap-3 border-b border-[var(--color-border)]/60">
                  ...
                  {/* Route */}
                  <div className="px-5 py-3.5 bg-[var(--color-badge-bg)]/40">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0 flex-1 text-right">
                        ...
                        <p className="text-[10px] text-[var(--color-muted)] flex items-center justify-end gap-1 mt-0.5">
                          <MapPin size={9} />
                          {ref.to_hospital?.location?.city}
                        </p>
                      </div>
                  ...
                  <div className="px-5 py-3.5 flex items-center justify-between gap-3">
                    ...
                    <Link
                      href={`/shared/referral/${ref.id}`}
                      className="inline-flex items-center gap-1.5 px-4 py-2 bg-[var(--color-badge-bg)] border border-[var(--color-border)] rounded-xl text-xs font-bold text-[var(--color-heading)] hover:border-[var(--color-accent)]/40 hover:text-[var(--color-accent)] hover:bg-white transition-all"
                    >
