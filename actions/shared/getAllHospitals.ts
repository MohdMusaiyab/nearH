...
> {
  ...
  if (locationId) {
    supabaseQuery = supabaseQuery.eq("location_id", locationId);
  }

  if (specialtyId) {
    supabaseQuery = supabaseQuery
      .not("doctors.specialty_id", "is", null)
      .filter("doctors.specialty_id", "eq", specialtyId);
  }
  ...
  const hospitals: ExploreHospital[] = (data as unknown as RawJoinResult[]).map(
    (h) => {
      ...
      return {
        ...
        has_ayushman_bharat: h.has_ayushman_bharat ?? false,
  ...
  return {
    ...
    data: { hospitals, totalCount: count || 0 },
  };
}
