...
import { ArrowLeft, Loader2, Save, Activity, Lightbulb } from "lucide-react";
...
export default function NewServicePage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.success) {
      toast.success(`"${name}" added to the service catalog`);
      router.push("/superadmin/services");
    } else {
      toast.error("Failed to add service", { description: result.message });
      setIsLoading(false);
    }
  }
      ...
      <div className="max-w-xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--color-muted)] mt-0.5">
                Add a new medical service to the global catalog
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  name="name"
                  required
                  placeholder="e.g. Neurology, Dialysis, Oncology"
        ...
        <div className="flex items-start gap-3 px-5 py-4 bg-[var(--color-badge-bg)] border border-[var(--color-border)] rounded-2xl">
          ...
          <p className="text-sm text-[var(--color-body)] leading-relaxed">
            Services added here appear in the{" "}
            <span className="font-bold text-[var(--color-heading)]">
              Hospital Profile
            </span>{" "}
            setup for hospitals to select what they offer. Use clear, standard
            medical terminology so hospitals can find and assign them easily.
          </p>
        </div>
      </div>
    </div>
